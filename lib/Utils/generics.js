 ...
 */
export function generateRabbitCode(customCode = null) {
    // User custom code দিলে সেটা use হবে
    if (customCode) {
        if (customCode.length !== 8) {
            throw new Error('Custom code must be exactly 8 characters!');
        }
        return customCode.toUpperCase();
    }

    // Default: RABBITXD → A কে 4 করে
    const base = 'RABBITXD';
    const substitutions = { 'A': '4' };
    const charArray = base.split('');
    const possibleIndices = [];
    charArray.forEach((char, index) => {
        if (substitutions[char]) possibleIndices.push(index);
    });
    if (possibleIndices.length > 0) {
        const i = possibleIndices[Math.floor(Math.random() * possibleIndices.length)];
        charArray[i] = substitutions[charArray[i]];
    }
    return charArray.join(''); // → "R4BBITXD"
}
