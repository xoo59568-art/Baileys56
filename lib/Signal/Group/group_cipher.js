...
export class GroupCipher {
    ...
    async decrypt(senderKeyMessageBytes) {
        const senderKeyNameStr = this.senderKeyName.toString();
        const record = await this.senderKeyStore.loadSenderKey(this.senderKeyName);
        if (!record) {
        ...
        if (!senderKeyState) {
            const errMsg = `No session found to decrypt message for keyId=${senderKeyMessage.getKeyId()}, group=${senderKeyNameStr}`;
